...
from mica_clusteringcoeff_pythonadaption_v3 import RandomizeClust, gretnapy_gen_random_network1_weight


def do_graphtheory_things(case, v=False):
	"""
	...
	Cw_group = bct.clustering_coef_wu(p)
	if v:
		print('clustering coefficient created for case. looks like this: ', Cw_group)
	Cw_rand_group = np.empty(shape=(rIt, p.shape[0]))*0

	for ii in range(rIt):
	    Cw_rand_group[ii] = RandomizeClust(parcel, 0.1)
	...
	Cw_group_norm[np.isnan(Cw_group_norm)] = 0

	# characteristic clustering coefficient
	#cCw_group = Cw_group_avg/np.mean(cGrand, axis=0)

	return(case, Cw_group_norm, Cw_group, Cw_rand_group, randomGs)
...
for case in cases:
	...
	results = do_graphtheory_things(case)
...
pd.DataFrame(d_cw_groupnorm).to_csv('graphtheory_abide_clusteringcoeff_groupnorm.csv')
pd.DataFrame(d_cw_group).to_csv('graphtheory_abide_clusteringcoeff_group.csv')
